...
import UserSearch from './user/containers/UserSearchContainer'
import IconUserSearch from './user/components/IconUserSearch'

import Privacy from './user/components/legal/privacy'
import Terms from './user/components/legal/terms'
...
const tab = ({ selected, title, iconName }) => {
  ...
  if(iconName){
    ...
    return (
        <Image source={require('../assets/images/tabbarspitch2.png')} style={styleSpitch}/>
    )
  }  
}
...
const Scenes = Actions.create(
    <Scene key='root' >
        ...
        <Scene key="tabbar" component={PushController} >
            <Scene key="tabbar2" tabs={true} style={{borderTopWidth:1, borderTopColor: '#cccccc', backgroundColor:'white'}} >
                ...
                <Scene {...sceneStyleTabbar} key="feed" title="Spitch" icon={tab} iconName="ios-square" component={ListFeed} 
                  titleStyle={{fontSize: 20, fontWeight:'500'}} renderRightButton={() => <IconNotification />} renderLeftButton={() => <IconUserSearch />}/>
        ...
        <Scene {...sceneStyleNoBorder} key="user_search" component={UserSearch} title={I18n.t('UserSearch_title')}  titleStyle={{fontSize: 18, fontWeight:'500'}} /> 

        <Scene {...sceneNavTransparent} key="visit" component={VisitProfile} leftButtonIconStyle={{'tintColor':'white'}} hideNavBar={false}/> 

        <Scene {...sceneNavTransparent} key="swipeask" component={SwipeAsk}  direction="vertical" hideNavBar renderBackButton={()=>(null)} 
        ...
        <Scene key="share" hideNavBar direction="vertical" component={Share}/>
        ...
        <Scene {...sceneStyle} key="privacy" title="Privacy Policy" component={Privacy} />
        <Scene {...sceneStyle} key="terms" title="Terms of services" component={Terms} />
...
//swipesask : renderRightButton={() => <CloseButton styles={{color:'white'}} onPress={() => Actions.pop()} />} 
