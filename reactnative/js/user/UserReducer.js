import { USER_AUTHENTICATED,
  ...
  REMOVE_LIST_USER_SPITCH,
  LIST_SEARCH_USER_PENDING, LIST_SEARCH_USER_FULFILLED, LIST_SEARCH_USER_REJECTED,
  NEXT_LIST_SEARCH_USER_PENDING, NEXT_LIST_SEARCH_USER_FULFILLED, NEXT_LIST_SEARCH_USER_REJECTED, LIST_SEARCH_USER_FOLLOW  } from './UserConstants'
...
const INITIAL_STATE = {
  ...
  search:{ pending:false, fulfilled:false, error:null, list:[], pagination:null, refresh:false, nextPending: false, nextFetched: false },
};
...
export default function(state = INITIAL_STATE, action) {
  ...
  switch(action.type) {
    ...
    case LIST_SEARCH_USER_PENDING:
        return { ...state, search: { ...state.search, pending: true, fulfilled: false, error:null, list:[] } }
    case LIST_SEARCH_USER_REJECTED:
        return { ...state, search: { ...state.search, pending: false, fulfilled: false, error: action.payload.response, list: [] } }
    case LIST_SEARCH_USER_FULFILLED:
        return { ...state, search: { ...state.search, pending: false, fulfilled: true, error: null, 
          list: action.payload.data.results, pagination:action.payload.data.next_cursor } }
    
    case NEXT_LIST_SEARCH_USER_PENDING:
        return { ...state, search: { ...state.search, nextPending: true, nextFetched:false } }
    case NEXT_LIST_SEARCH_USER_REJECTED:
        return { ...state, search: { ...state.search, nextPending: false, nextFetched:false,} }
    case NEXT_LIST_SEARCH_USER_FULFILLED:
        return { ...state, search: { ...state.search, nextPending: false, nextFetched:true, 
          list: state.search.list.concat(action.payload.data.results), pagination:action.payload.data.next_cursor } }


    case LIST_SEARCH_USER_FOLLOW:
        return { 
            ...state,
            search: { ...state.search, 
                    list: state.search.list.map(function(item) {
                         if(item.id == action.id){
