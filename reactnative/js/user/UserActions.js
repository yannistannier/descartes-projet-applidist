...
import { USER_AUTHENTICATED, RETREIVE_USER, UPDATE_USER, RETREIVE_USER_DATAS, LIST_USER_SPITCH, NEXT_LIST_USER_SPITCH, 
	LIST_USER_ASK, NEXT_LIST_USER_ASK, LIST_SEARCH_USER, NEXT_LIST_SEARCH_USER, LIST_SEARCH_USER_FOLLOW } from './UserConstants'
...
export const listSearchUser = (search=null) => {
  return {
      type: LIST_SEARCH_USER,
      payload: api.get('user/search/'+(search ? "?search="+search : ''))
  }
}

export const nextListSearchUser = (cursor, search=null) => {
  return {
      type: NEXT_LIST_SEARCH_USER,
      payload: api.get('user/search/?cursor='+cursor+(search ? "&search="+search : ''))
  }
}

export const followUnfollowListSearchUser = (id) => {
  return {
      type: LIST_SEARCH_USER_FOLLOW,
      id
  }
}
